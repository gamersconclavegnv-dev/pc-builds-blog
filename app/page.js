...
function scoreGPU(gpuStr) {
  for (const tier of GPU_TIERS) {
    if (tier.match.test(gpuStr)) return tier.score;
  }
  return 500; // unknown GPU
}

function scoreCPU(cpuStr) {
  for (const tier of CPU_TIERS) {
    if (tier.match.test(cpuStr)) return tier.score;
  }
  return 800; // unknown CPU
}

function scoreRAM(gb) {
  const keys = Object.keys(RAM_SCORES).map(Number).sort((a, b) => a - b);
  for (let i = keys.length - 1; i >= 0; i--) {
    if (gb >= keys[i]) return RAM_SCORES[keys[i]];
  }
  return 100;
}
...
function scoreCores(n) {
  ...
  for (let i = keys.length - 1; i >= 0; i--) {
    if (n >= keys[i]) return CORE_SCORES[keys[i]];
  }
  return 100;
}
...
export default function Home() {
  ...
  // benchmark state
  const [benchState, setBenchState]       = useState('idle'); // idle | collecting | running | done
    ...
    if (data?.length) {
      ...
      data.forEach(build => {
        try {
          ...
          if (parts.photos?.length) parts.photos.forEach(p => { if (p) photos.push(p); });
          else if (parts.photo) photos.push(parts.photo);
        } catch {}
      });
  ...
  // Step 1: collect CPU name from user, auto-detect the rest
  ...
  // Step 2: run the animation then score
  const runBenchmark = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        const gpuScore  = scoreGPU(detectedSpecs.gpu);
        const cpuScore  = scoreCPU(cpuInput);
        const ramScore  = scoreRAM(detectedSpecs.ram);
    ...
    await supabase.from('benchmark_scores').upsert(
      {
        ...
        cores:    detectedSpecs.cores,
        ram_gb:   detectedSpecs.ram,
        platform: detectedSpecs.platform,
        score:    benchResult.total,
      },
      { onConflict: 'user_id' }
    );
    setSubmitting(false);
    ...
    <main style={{ backgroundColor: '#0a0a0a', minHeight: '100vh', fontFamily: '"Courier New", Courier, monospace', color: '#00ff00' }}>
      ...
      <div style={{ borderBottom: '1px solid #003300' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto', textAlign: 'center', padding: '60px 20px' }}>
          <h1 style={{ fontSize: 'clamp(28px,4vw,56px)', fontWeight: 'bold', letterSpacing: '4px', margin: '0 0 10px', textShadow: '0 0 10px #00ff00' }}>
            GAMER&apos;S CONCLAVE
          </h1>
        ...
        <div style={{ overflow: 'hidden', width: '100%' }} onMouseEnter={() => { pausedRef.current = true; }} onMouseLeave={() => { pausedRef.current = false; }}>
          {loading && (
            <div style={{ display: 'flex', gap: '12px', padding: '0 12px' }}>
              {Array.from({ length: 8 }).map((_, i) => <div key={i} className="skel-card" style={{ animationDelay: `${i*0.15}s` }}><div className="skel-label"/></div>)}
            </div>
          )}
            ...
            <div ref={trackRef} className="reel-fade-in" style={{ display: 'flex', gap: '12px', width: 'max-content', willChange: 'transform' }}>
              {loopImages.map((src, i) => (
                <div key={i} style={{ flexShrink: 0, width: '320px', height: '210px', border: '1px solid #002200', overflow: 'hidden', position: 'relative' }}>
                  <img src={src} alt="community build" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', filter: 'brightness(0.88)' }} onError={e => { e.currentTarget.parentElement.style.display = 'none'; }} />
          ...
          {!loading && scrollImages.length === 0 && (
            <div style={{ textAlign: 'center', color: '#004400', fontSize: '13px', padding: '40px', letterSpacing: '1px' }}>&gt; NO BUILDS YET — BE THE FIRST TO POST YOUR RIG_</div>
          )}
        </div>
      </div>
      ...
      <div style={{ borderBottom: '1px solid #003300' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '40px 20px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(300px,560px) 1fr', gap: '32px', alignItems: 'start', flexWrap: 'wrap' }}>
                ...
                {benchState === 'idle' && (
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: '11px', color: '#006600', marginBottom: '16px', lineHeight: '1.8' }}>
                      SCANS YOUR ACTUAL GPU, CPU CORES &amp; RAM<br/>
                      <span style={{ color: '#004400' }}>ENTER YOUR CPU NAME TO BEGIN</span>
                    </div>
...
\\___)=(___/
  TUX AWAITS`}</pre>
                ...
                {/* COLLECTING CPU INPUT */}
                {benchState === 'collecting' && (
                  <div>
                    <div style={{ fontSize: '12px', color: '#009900', marginBottom: '16px', lineHeight: '1.8' }}>
                      ✓ GPU DETECTED: <span style={{ color: '#00ff00' }}>{detectedSpecs.gpu}</span><br/>
                      ✓ CPU CORES: <span style={{ color: '#00ff00' }}>{detectedSpecs.cores}</span><br/>
                      ✓ RAM: <span style={{ color: '#00ff00' }}>{detectedSpecs.ram}GB</span><br/>
                      ✓ PLATFORM: <span style={{ color: '#00ff00' }}>{detectedSpecs.platform}</span>
                    </div>
                    <div style={{ fontSize: '11px', color: '#ff4444', marginBottom: '12px' }}>
                      ⚠ BROWSERS CANNOT READ CPU NAME — ENTER BELOW:
                    </div>
                    <label style={{ fontSize: '11px', color: '#006600', letterSpacing: '1px', display: 'block' }}>YOUR CPU</label>
                    <input
                      style={inputStyle}
                      value={cpuInput}
                      onChange={e => setCpuInput(e.target.value)}
                      placeholder="e.g. Ryzen 7 5800X or i7-13700K"
                      onKeyDown={e => { if (e.key === 'Enter') runBenchmark(); }}
                    />
                  ...
                  <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px', marginTop: '16px' }}>
                      {BENCH_STAGES.map((stage, i) => {
                        const current = Math.floor((benchProgress / 100) * BENCH_STAGES.length);
                        return <div key={i} style={{ fontSize: '10px', color: i <= current ? '#006600' : '#002200', letterSpacing: '1px' }}>{i <= current ? '▓' : '░'} {stage}</div>;
                      })}
                    </div>
                  </div>
                )}
...
/'\\_   _/\`\\
\\___)=(___/`}</pre>
                    <div className="bench-result" style={{ fontSize: '22px', fontWeight: 'bold', letterSpacing: '3px', color: benchResult.color, marginBottom: '6px' }}>
                      {benchResult.rank}
                    </div>
                    <div style={{ fontSize: '28px', fontWeight: 'bold', color: '#00ff00', marginBottom: '16px' }}>
                      {benchResult.total.toLocaleString()} pts
                    </div>
                    ...
                    <div style={{ textAlign: 'left', border: '1px solid #003300', padding: '12px', backgroundColor: '#050505', marginBottom: '16px' }}>
                      ...
                      {[
                        { label: 'GPU', value: benchResult.gpuScore, detail: detectedSpecs.gpu },
                        { label: 'CPU', value: benchResult.cpuScore, detail: cpuInput },
                        { label: 'RAM', value: benchResult.ramScore, detail: `${detectedSpecs.ram}GB` },
                    ...
                    <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
                      ...
                      <button onClick={() => { setBenchState('idle'); setCpuInput(''); setBenchResult(null); setSubmitted(false); }}
                  ...
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                    ...
                    <tbody>
                      {leaderboard.map((row, i) => {
                        ...
                        return (
                          <tr key={row.id} className="lb-row" style={{ borderBottom: '1px solid #001100', cursor: 'default' }}>
